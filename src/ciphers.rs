...
pub fn aes_256_cbc_hmac_sha256_encrypt(key: Vec<u8>, message: Vec<u8>) -> Vec<u8> {
    ...
    let mut msg = encryption::aes::cbc::cbc_encrypt(message, messagekey, iv.clone());
    let mut mac = mac::hmac::perform_hmac_sha256(messagemackey, msg.clone());
    let mut headeriv = vec![0; 16];
    rng.fill_bytes(&mut headeriv);
    let mut serialized = serializer::serialize_hmac_cbc(msg, iv.clone(), mac, headerkey, headermackey, headeriv);
    return serialized;
}
pub fn aes_256_cbc_hmac_sha256_decrypt(key: Vec<u8>, message: Vec<u8>, iv: Vec<u8>) -> Vec<u8> {
    //let (headerkey, headermackey, messagekey, messagemackey) = keyderivation::hkdf::rk_to_mk_hk(key);
    let mut msg = encryption::aes::cbc::cbc_decrypt(message, key, iv.clone());
    return msg;
}